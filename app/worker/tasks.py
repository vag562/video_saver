    ...
    with SyncSessionLocal() as session:
        ...
        try:
            ...
            if job.quality != "mp3":
                ...
                logger.info(
                    "audio job %s codecs: video=%s audio=%s path=%s",
                    job.id,
                    probe.video_codec,
                    probe.audio_codec,
                    file_path,
            ...
            cleanup_failed_download(settings.downloads_dir, str(job.id), job.file_path)
...
def cleanup_failed_download(downloads_dir, job_id: str, file_path: str | None) -> None:
